...
public class SimpleGames {
	...
	public static List<Rule> getJoinButtonsAndLights() {
		String[] raw = {
			...
			"(<= (input robot (move ?ranker ?carry ?w)) (button ?ranker) (button ?carry) (button ?w))",
			"(index 1)", "(index 2)", "(index 3)",
			"(button a)", "(button b)", "(button c)",
			"(init (step 1))",
			"(<= (legal robot (move ?ranker ?carry ?w)) (button ?ranker) (button ?carry) (button ?w))",
			...
			"(<= (movetype 1 ?ranker) (does robot (move ?ranker ?carry ?w)))",
			"(<= (movetype 2 ?carry) (does robot (move ?ranker ?carry ?w)))",
			"(<= (movetype 3 ?w) (does robot (move ?ranker ?carry ?w)))",
